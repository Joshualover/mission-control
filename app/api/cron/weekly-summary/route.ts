...
// Cron schedule: 每周一 9:00 AM (UTC) 发送
// 配置在 vercel.json 中
export async function GET(request: NextRequest) {
  try {
    ...
    console.log(`Found ${subscriptions.length} active subscriptions for weekly summary`);
    ...
    const weekRange = `${startOfWeek.toLocaleDateString('zh-CN', { month: 'short', day: 'numeric' })} - ${endOfWeek.toLocaleDateString('zh-CN', { month: 'short', day: 'numeric' })}`;
    ...
    for (const subscription of subscriptions) {
      try {
        ...
        if (result.success) {
          successCount++;
          console.log(`✓ Weekly summary sent to ${subscription.email}`);
        } else {
          failedCount++;
          console.error(`✗ Failed to send weekly summary to ${subscription.email}`);
        }
      } catch (error) {
        failedCount++;
